  ...
  return(
    <Modal title="⬡ Export as Hierarchy Block" onClose={()=>setShowHierarchyExporter(false)} width={520}>
      ...
      <div style={{display:'flex',justifyContent:'flex-end',gap:8}}>
        <GhostBtn onClick={()=>useStore.getState().setShowHierarchyExporter(false)}>Cancel</GhostBtn>
...
// ── PDKSelector (sky130 + gf180) ────────────────────────────────────────────
const FALLBACK=[
  {id:'sky130',name:'SkyWater SKY130',process:'130nm',color:'#38bdf8',enabled:true,description:'Open-source 130nm CMOS. 45-cell curated set.',downloadUrl:'https://github.com/google/skywater-pdk'},
  {id:'gf180',name:'GF180MCU',process:'180nm',color:'#fb923c',enabled:true,description:'GlobalFoundries 180nm open PDK. 30-cell curated set.',downloadUrl:'https://github.com/google/gf180mcu-pdk'},
...
export function PDKSelector({onComplete}){
  ...
  const defaultPathHint={sky130:'/pdk/sky130/versions/0fe599b2…',gf180:'/pdk/gf180mcuD',ihp130:'/pdk/ihp-sg13g2'};
  return(
    <div style={{position:'fixed',inset:0,background:'#000000cc',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,fontFamily:"'Space Grotesk',sans-serif"}}>
      <div style={{background:'#161b27',border:'1px solid #1e2733',borderRadius:12,width:680,maxHeight:'90vh',overflow:'hidden',display:'flex',flexDirection:'column',boxShadow:'0 24px 64px rgba(0,0,0,.6)'}}>
                  ...
                  {picked===pdk.id&&pdk.enabled&&(
                    <div onClick={e=>e.stopPropagation()} style={{marginTop:10,paddingTop:10,borderTop:'1px solid #1e2733'}}>
                      ...
                      <div style={{display:'flex',gap:6}}>
                        <input value={paths[pdk.id]||''} onChange={e=>setPaths(p=>({...p,[pdk.id]:e.target.value}))} placeholder={defaultPathHint[pdk.id]||'/pdk/...'} style={{flex:1,background:'#0d1117',border:'1px solid #2d3748',borderRadius:4,color:'#e2e8f0',fontSize:11,padding:'5px 8px',fontFamily:"'JetBrains Mono',monospace",outline:'none'}}/>
                      ...
                      <div style={{color:'#334155',fontSize:10,marginTop:6,fontFamily:"'JetBrains Mono',monospace"}}>In Docker: {defaultPathHint[pdk.id]}</div>
                    </div>
                  )}
                </div>
              ))}
        ...
        <div style={{padding:'16px 24px',borderTop:'1px solid #1e2733',display:'flex',justifyContent:'flex-end',gap:10}}>
          ...
          <button onClick={handleConfirm} disabled={loading||!picked} style={{background:'#3b82f6',border:'none',borderRadius:6,color:'#fff',fontSize:13,fontWeight:600,padding:'8px 24px',cursor:loading?'wait':'pointer',opacity:loading?.7:1}}>{loading?'Loading PDK…':isLocked?'Apply':'Create Project'}</button>
        </div>
      </div>
    </div>
  );
...
function Modal({title,onClose,children,width=580}){
  return(<div style={{position:'fixed',inset:0,background:'#000000cc',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,fontFamily:"'Space Grotesk',sans-serif"}}>
