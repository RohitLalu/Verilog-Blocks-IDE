...
// PDK
ipcMain.handle('pdk:list', async () => {
  const reg = readJSON(path.join(__dirname, 'pdk', 'pdk-registry.json'));
  return reg;
});
...
ipcMain.handle('pdk:validate-path', async (_, { pdkId, rootPath }) => {
  ...
  const expected = { sky130: ['sky130_fd_sc_hd.v','sky130_fd_sc_hd__tt_025C_1v80.lib'], gf180: ['gf180mcu_fd_sc_mcu7t5v0.v'], ihp130: ['sg13g2_stdcell.v'] };
  const found = {};
...
ipcMain.handle('project:list', async () => {
  ...
  return fs.readdirSync(PROJ).filter(f => fs.statSync(path.join(PROJ, f)).isDirectory());
});
...
ipcMain.handle('project:load', async (_, name) => {
  ...
  return {
    ...
    verilog:   fs.readFileSync(path.join(dir, 'top.v'), 'utf8'),
...
ipcMain.handle('yosys:synth', async (_, { projectName, verilog, pdkRoot, pdkId }) => new Promise(resolve => {
  ...
  script += ['hierarchy -top top','proc; opt; fsm; opt; memory; opt', pdkId==='sky130'?'synth_sky130 -top top':'synth -top top', `write_verilog "${path.join(outDir,'top_synth.v')}"`, `write_json "${path.join(outDir,'top_synth.json')}"`, 'stat'].join('\n');
