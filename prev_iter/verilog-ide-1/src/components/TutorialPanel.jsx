...
const KW = ['module','endmodule','input','output','wire','reg','assign','always','posedge','negedge','if','else','begin','end','case','endcase','parameter'];

function VHL({ code }) {
  return (
    <>{code.split('\n').map((line, i) => {
      if (line.trim().startsWith('//')) return <span key={i} style={{ color: '#2d4a2d' }}>{line}{'\n'}</span>;
      if (line.trim().startsWith('`'))  return <span key={i} style={{ color: '#4c1d95' }}>{line}{'\n'}</span>;
      return <span key={i}>{line.split(/(\b\w+\b|'[01xz]|"[^"]*"|#\d+)/).map((t, j) => KW.includes(t) ? <span key={j} style={{ color: '#60a5fa' }}>{t}</span> : /^\d+$/.test(t) ? <span key={j} style={{ color: '#f59e0b' }}>{t}</span> : t)}{'\n'}</span>;
    })}</>
  );
}

export default function TutorialPanel() {
  const b = useStore(s => s.tutorialBlock);
    ...
    <div style={{ width: 340, background: '#0d1117', borderLeft: '1px solid #1e2733', display: 'flex', flexDirection: 'column', fontFamily: "'Space Grotesk',sans-serif", flexShrink: 0 }}>
      <div style={{ padding: '12px 16px 0', borderBottom: '1px solid #1e2733' }}>
        ...
        <div style={{ display: 'flex' }}>
          {tabs.map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ background: 'transparent', border: 'none', borderBottom: `2px solid ${tab===t?color:'transparent'}`, color: tab===t?color:'#334155', fontSize: 11, fontWeight: tab===t?600:400, padding: '5px 12px 8px', cursor: 'pointer', fontFamily: "'Space Grotesk',sans-serif", textTransform: 'capitalize' }}>{t}</button>
          ))}
        </div>
      </div>
      <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        ...
        {tab === 'code' && <>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            ...
            <button onClick={() => { navigator.clipboard.writeText(snippet); setCopied(true); setTimeout(()=>setCopied(false),1500); }}
              style={{ background: '#161b27', border: '1px solid #1e2733', borderRadius: 4, color: '#475569', fontSize: 10, padding: '2px 8px', cursor: 'pointer' }}>{copied ? '✓' : 'copy'}</button>
          </div>
          <pre style={{ background: '#0a0e18', border: '1px solid #1e2733', borderRadius: 6, padding: 12, margin: 0, fontSize: 11, lineHeight: 1.8, color: '#94a3b8', whiteSpace: 'pre-wrap', fontFamily: "'JetBrains Mono',monospace" }}>
            <VHL code={snippet} />
          </pre>
        </>}
        {tab === 'ports' && (
        ...
        {tab === 'timing' && (() => {
          ...
          if (!t) return <div style={{ color: '#334155', fontSize: 12 }}>No timing data for this cell.</div>;
          const rows = [];
          if (t.tpd_typ) rows.push({ label:'Propagation delay (typ)', val:`${t.tpd_typ} ns`, note:'TT 1.8V 25°C' });
          if (t.tsu)     rows.push({ label:'Setup time',              val:`${t.tsu} ns`,     note:'CLK→D' });
          if (t.thd)     rows.push({ label:'Hold time',               val:`${t.thd} ns`,     note:'CLK→D' });
          if (t.tco)     rows.push({ label:'Clock-to-Q delay',        val:`${t.tco} ns`,     note:'typ' });
          return (<>
            <div style={{ color:'#334155',fontSize:10,marginBottom:10 }}>TT corner · sky130_fd_sc_hd · 1.8V 25°C</div>
            <table style={{ width:'100%',borderCollapse:'collapse',fontFamily:"'JetBrains Mono',monospace" }}>
              <tbody>{rows.map(r=><tr key={r.label} style={{ borderBottom:'1px solid #0f1419' }}><td style={{ color:'#475569',fontSize:11,padding:'6px 6px' }}>{r.label}</td><td style={{ color,fontSize:12,padding:'6px 6px',fontWeight:700 }}>{r.val}</td><td style={{ color:'#1e2733',fontSize:10,padding:'6px 6px' }}>{r.note}</td></tr>)}</tbody>
            </table>
          </>);
        })()}
      </div>
