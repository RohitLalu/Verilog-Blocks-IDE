...
const POWER=new Set(['VPWR','VGND','VPB','VNB','VDD','VSS']);
export function buildNetlist(nodes,edges){
  ...
  nodes.forEach(node=>{
    ...
    cells.set(node.data.instanceName,{cellId,instanceName:node.data.instanceName,inputs:inputNets,outputs:outputNets,isFF:isSequential(cellId),isComb:isCombinational(cellId),state:{Q:'x'},prevCLK:'x',prevCLK_n:'x'});
  });
