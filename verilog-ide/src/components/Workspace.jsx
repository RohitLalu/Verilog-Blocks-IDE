...
function WorkspaceInner(){
  const{nodes,edges,onNodesChange,onEdgesChange,onConnect,addBlock,setPDKLocked,undo,redo}=useStore();
  const wrapRef=useRef(null);const{screenToFlowPosition}=useReactFlow();
  const onDrop=useCallback(e=>{
    e.preventDefault();const raw=e.dataTransfer.getData('application/blockdef');if(!raw)return;
    const blockDef=JSON.parse(raw);const pos=screenToFlowPosition({x:e.clientX,y:e.clientY});
    addBlock(blockDef,{x:pos.x-75,y:pos.y-40});setPDKLocked(true);
  ...
  return(
    <div ref={wrapRef} style={{flex:1,height:'100%'}} onDrop={onDrop} onDragOver={onDragOver} onKeyDown={onKeyDown} tabIndex={0}>
      <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect}
        ...
        <MiniMap style={{background:'#0d1117',border:'1px solid #1e2733',borderRadius:7}} nodeColor={n=>n.data?.color||'#1e2733'} maskColor="#0d111788"/>
        {nodes.length===0&&<div style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-50%)',textAlign:'center',pointerEvents:'none',fontFamily:"'Space Grotesk',sans-serif"}}>
          <div style={{fontSize:52,opacity:.06,marginBottom:12}}>⬡</div>
