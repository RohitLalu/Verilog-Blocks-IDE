...
function useTopInputs(nodes,edges){
  return useMemo(()=>{
    const by={};edges.forEach(e=>{by[`${e.target}:${e.targetHandle}`]=true;});
    const seen=new Set(),ports=[];
    nodes.forEach(node=>{
      node.data.ports?.inputs?.forEach(p=>{
        if(!by[`${node.id}:${p.name}`]){
          const sig=`${node.data.instanceName}_${p.name}`;
          if(!seen.has(sig)){seen.add(sig);ports.push({sig,portName:p.name,width:p.width});}
        }
      });
    });
    return ports;
...
const TABS={inputs:'Inputs',dft:'DFT / Scan',preview:'Preview',custom:'Custom TB'};
...
export default function TestbenchPanel(){
  const{nodes,edges,projectName,testbenchInputs,setTestbenchInput,clearTestbenchInputs,
    ...
    useCustomTestbench,setUseCustomTestbench,simOutput,simRunning,setSimRunning,
    setSimOutput,setLastVCD,lastVCD,setActivePanel,
    generatedVerilog,pdkContext,pushNotification}=useStore();
  ...
  const ffNodes=useMemo(()=>nodes.filter(n=>
    ['sky130_dfxtp','sky130_dfrtp','sky130_dfstp','sky130_edfxtp','sky130_sdfxtp','sky130_sdfrtp',
     'gf180_dff','gf180_dffr','gf180_dffs','gf180_dffsr','gf180_dffn','gf180_sdff'].includes(n.data.id)
  ),[nodes]);
  ...
  const handlePreview=()=>{
    ...
    setGeneratedTestbench(tb);setTab('preview');
  };

  // ── Run iverilog → auto-navigate to SimulatorPanel with VCD ───────────────
  const handleRun=async()=>{
    if(!window.api){
      pushNotification('iverilog requires Docker — use docker compose up -d','warn');
      return;
    }
    const verilog=generatedVerilog;
    if(!verilog||verilog.trim()==='// Empty design\nmodule top();\nendmodule\n'){
      pushNotification('Generate Verilog first (</> button)','warn');
      return;
    }
    const tb=generateTestbench(nodes,edges,projectName,testbenchInputs,useCustomTestbench?customTestbench:'');
    setGeneratedTestbench(tb);
    setSimRunning(true);setSimOutput('');
    ...
    try{
      ...
      if(res.vcd){
        // Pipe VCD directly into the SimulatorPanel VCD viewer and navigate there
        setLastVCD(res.vcd);
        setActivePanel('simulator');   // auto-navigate — user lands on waveforms immediately
        pushNotification('Simulation complete ✓','success');
      }else{
        pushNotification(res.ok?'Simulation done (no VCD)':'Simulation error','error');
        setActivePanel('simulator');   // still navigate — console output visible there
      }
    }catch(e){
      setSimOutput('Request failed: '+e.message);
      pushNotification('Simulation request failed','error');
    }
    setSimRunning(false);
  };
  ...
  return(
    <div style={{width:380,background:'#0d1117',borderLeft:'1px solid #1e2733',display:'flex',flexDirection:'column',fontFamily:"'Space Grotesk',sans-serif",flexShrink:0}}>
      ...
      <div style={{padding:'12px 16px 0',borderBottom:'1px solid #1e2733'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            ...
            <button onClick={handleRun} disabled={simRunning||!nodes.length}
              style={{background:'#0a2218',border:'1px solid #166534',borderRadius:6,color:'#4ade80',
                fontSize:12,fontWeight:700,padding:'5px 14px',cursor:simRunning||!nodes.length?'not-allowed':'pointer',
                opacity:simRunning||!nodes.length?.4:1}}>
              {simRunning?'⟳ Running…':'▶ iverilog'}
            </button>
          </div>
        </div>
        {simRunning&&<div style={{color:'#334155',fontSize:10,marginBottom:8,fontFamily:"'JetBrains Mono',monospace"}}>
          Running iverilog… waveforms will appear automatically when done.
        </div>}
        <div style={{display:'flex'}}>
          {Object.entries(TABS).map(([k,v])=>(
            <button key={k} onClick={()=>setTab(k)} style={{background:'transparent',border:'none',
              borderBottom:`2px solid ${tab===k?'#f472b6':'transparent'}`,color:tab===k?'#f472b6':'#334155',
              fontSize:11,padding:'5px 12px 8px',cursor:'pointer',fontFamily:"'Space Grotesk',sans-serif"}}>{v}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{flex:1,overflowY:'auto',padding:16}}>
        ...
        {/* ── Inputs tab ── */}
        {tab==='inputs'&&<>
          <div style={{color:'#475569',fontSize:11,marginBottom:12,lineHeight:1.6}}>
            Configure each top-level input. Click <strong style={{color:'#4ade80'}}>▶ iverilog</strong> above to run — waveforms open automatically.
          </div>
          {ports.length===0&&<div style={{color:'#1e2733',fontSize:12,textAlign:'center',padding:24}}>
            No top-level inputs detected.<br/><span style={{fontSize:10}}>Place blocks and leave input ports unconnected.</span>
          </div>}
          {ports.map(port=>(
          ...
          {ports.length>0&&<div style={{marginTop:10,display:'flex',gap:8}}>
        ...
        {tab==='dft'&&<>
          <div style={{color:'#475569',fontSize:11,lineHeight:1.7,marginBottom:14}}>
            Scan insertion replaces DFFs with scan variants, chaining them into a shift register for test access.
            <br/><span style={{color:'#1e2733',fontSize:10}}>Real scan rewriting (DFF→SDFF in generated Verilog) is not yet implemented — this shows the chain structure.</span>
          </div>
          ...
          {dftOn&&<>
            <div style={{marginBottom:14}}>
              <div style={{color:'#334155',fontSize:10,textTransform:'uppercase',letterSpacing:1,marginBottom:8}}>
                Flip-Flops ({ffNodes.length})
              </div>
              {ffNodes.length===0&&<div style={{color:'#1e2733',fontSize:11}}>No flip-flops in design.</div>}
              {ffNodes.map(n=><div key={n.id} style={{display:'flex',alignItems:'center',gap:8,padding:'5px 0',borderBottom:'1px solid #0f1419'}}>
                <input type="checkbox" defaultChecked/>
                <span style={{color:'#94a3b8',fontSize:11,fontFamily:"'JetBrains Mono',monospace"}}>{n.data.instanceName}</span>
                <span style={{color:'#1e3a5f',fontSize:9}}>→ {n.data.id.startsWith('gf180')?'sdff_1':'sdfxtp_1'}</span>
              </div>)}
            </div>
            {ffNodes.length>0&&<div style={{marginBottom:14}}>
            ...
            <div style={{background:'#111827',border:'1px solid #1e2733',borderRadius:7,padding:12,marginBottom:14}}>
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                <span style={{color:'#475569',fontSize:11}}>Est. test vectors:</span>
              ...
              <div style={{color:'#1e2733',fontSize:9.5,marginTop:5}}>
                Formula-based estimate. For real ATPG run atalanta (◈ ATPG panel).
              </div>
            </div>
          </>}
        </>}
        ...
        {tab==='preview'&&<>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
            ...
            <button onClick={()=>navigator.clipboard.writeText(generatedTestbench)}
              style={{background:'#161b27',border:'1px solid #1e2733',borderRadius:4,color:'#475569',fontSize:10,padding:'2px 8px',cursor:'pointer'}}>copy</button>
          </div>
          <pre style={{background:'#0a0e18',border:'1px solid #1e2733',borderRadius:6,padding:12,margin:0,
            fontSize:10.5,lineHeight:1.8,color:'#64748b',whiteSpace:'pre-wrap',wordBreak:'break-word',
            fontFamily:"'JetBrains Mono',monospace",maxHeight:480,overflowY:'auto'}}>
            {generatedTestbench||'Click "Preview TB" in Inputs tab to generate.'}
          </pre>
        </>}
        ...
        {tab==='custom'&&<>
          ...
          <label style={{display:'flex',alignItems:'center',gap:8,marginBottom:12,cursor:'pointer'}}>
            ...
            <span style={{color:'#e2e8f0',fontSize:12}}>Use custom testbench when running iverilog</span>
          </label>
          <textarea value={customTestbench} onChange={e=>setCustomTestbench(e.target.value)}
            placeholder={"`timescale 1ns / 1ps\nmodule top_tb;\n    // your testbench here\nendmodule"}
            style={{width:'100%',height:320,background:'#0a0e18',border:'1px solid #1e2733',borderRadius:6,
              color:'#94a3b8',fontSize:10.5,fontFamily:"'JetBrains Mono',monospace",padding:12,
...
function PortCfg({port,config,onChange}){
  ...
  return(<div style={{background:'#111827',border:'1px solid #1e2733',borderRadius:7,padding:12,marginBottom:10}}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
      <div>
    ...
    <div style={{display:'flex',gap:4,marginBottom:10}}>
      {types.map(t=><button key={t} onClick={()=>onChange({...c,type:t})}
        style={{background:c.type===t?`${tc[t]}22`:'transparent',border:`1px solid ${c.type===t?tc[t]:'#1e2733'}`,
          borderRadius:4,color:c.type===t?tc[t]:'#334155',fontSize:9.5,padding:'3px 7px',cursor:'pointer'}}>{t}</button>)}
    </div>
    ...
    {c.type==='random'&&<Row label="Count:"><input type="number" min={1} value={c.count??8} onChange={e=>onChange({...c,count:parseInt(e.target.value)||8})} style={ninp}/><span style={{color:'#334155',fontSize:10}}>steps @ {c.period||10}ns each</span></Row>}
  </div>);
}
const Row=({label,children})=><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:5}}><span style={{color:'#475569',fontSize:11}}>{label}</span>{children}</div>;
